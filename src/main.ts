...
import { make, encodePNGToStream, Context, Bitmap } from "pureimage";
import parseFile from "./parsing";
import { MyInfo, AnimeInfo, MyAnimeListData, parsedXML, STATUS } from "./types";
...
function main() {
	...
	// console.log(`Found data for user ${data.myinfo.user_name}`);
	// data.anime.forEach((anime) => {
	// 	// console.log(`Found anime: ${anime}`);
	// 	console.log(`Found anime: ${anime.series_title}`);
	// 	console.log(`\tstatus: ${anime.my_status}`);
	// 	console.log(`\t start: ${anime.my_start_date}`);
	// 	console.log(`\t   end: ${anime.my_finish_date}`);
	// });

	createPNGFromData(data, 'out.png');
}

function createPNGFromData(data: MyAnimeListData, path: string) {
